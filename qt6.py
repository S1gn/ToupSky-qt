        ...
        if self.hcam:
            if 0 == self.cur.model.still:    # not support still image capture
                if self.pData is not None:
                    image = QImage(self.pData, self.imgWidth, self.imgHeight, toupcam.TDIBWIDTHBYTES(self.imgWidth * 24), QImage.Format.Format_RGB888)
                    self.count += 1
        ...
        try:
            ...
            if info.width > 0 and info.height > 0:
                ...
                try:
                    ...
                    image = QImage(buf, info.width, info.height, toupcam.TDIBWIDTHBYTES(info.width * 24), QImage.Format.Format_RGB888)
                    self.count += 1
                    image.save("pyqt{}.jpg".format(self.count))
